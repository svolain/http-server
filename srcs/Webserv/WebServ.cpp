...
/*   Updated: 2024/09/27 18:16:23 by klukiano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool WebServ::accept_new_connection(int sock, short revents, std::vector<pollfd> &pollFDs_)
{
    ...
    if ((*it).get_listening().fd == sock && (revents & POLLIN)){
      ...
      if (newClient.fd != -1){
        ...
        if (setsockopt(newClient.fd, SOL_SOCKET, MSG_NOSIGNAL, &yes,
            sizeof(int)) == -1) {
          std::cerr << "accept_new_connection: setsockopt() error"  << std::endl; 
        }
      }
      else
...
void WebServ::poll_available_fds(void){
  for (size_t i = 0; i < pollFDs_.size(); i++){
    ...
    if (!fd_info && accept_new_connection(fd, revents, pollFDs_)){
      continue;
    }
    else if (revents & POLLHUP){
      std::cout << "hang up" << fd << " with i = " << i  << std::endl;
      close_connection(fd, i, pollFDs_);
      i --;
      continue;
    }
      ...
      if (vhosts_it != (*v_hosts_).end()){
        ...
        vhosts_it->second.on_message_recieved(fd_info, pollFDs_[i]);
      }
      else {
...
void WebServ::close_connection(int sock, int i, std::vector<pollfd> &pollFDs_)
{
  close(sock);
  pollFDs_.erase(pollFDs_.begin() + i);
